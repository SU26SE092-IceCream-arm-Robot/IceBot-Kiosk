...
class ClientDeviceSessionManager extends ChangeNotifier {
  ...
  Future<ClientDeviceRuntimeIdentity?> _exchange() async {
    ...
    try {
      ...
      if (rawDevice is! Map || expiresAt == null) {
        throw const ApiException(
          type: ApiErrorType.unknown,
          message: 'Phản hồi xác thực thiết bị không hợp lệ.',
        );
      }
      ...
      if (identity.kioskId.trim().isEmpty ||
          identity.accessToken.trim().isEmpty) {
        throw const ApiException(
          type: ApiErrorType.unknown,
          message: 'Phản hồi xác thực thiết bị không hợp lệ.',
        );
  ...
  void clearIdentity() => _setIdentity(null);
...
Map<String, dynamic> _readData(Object? response) {
  ...
  if (envelope['succeeded'] != true || data is! Map) {
    throw ApiException(
      ...
      statusCode: envelope['statusCode'] as int? ?? 401,
      message:
          envelope['message'] as String? ?? 'Thiết bị chưa được cấp quyền.',
    );
  }
